...
#create sets for ASSOCIATE degreee, Information Technology degree:
info_req = { 'CSC 110', 'ENG 111',  'ENG 112', 'ETR 149', 'ETR 164', 'ITDF 110',
                 'ITD 132', 'ITE 182', 'ITE215', 'ITN 101', 'ITN 106' 'ITN 111'
                 'ITP 120', 'MTH 131',}
...
def process_network():
    global network_elect #this set must global since it is CHANGED in this function
    global num_net_req, num_net__elect, tot_net, all_net_courses
    temp_set = set() #create a temporary set empty set

    for course in network_elect:
        asterick_course = "*" + course #insert an asterisk in front fo ELECTIVE course name
        ...
        network_elect = temp_set.copy() #COPY all teh courses from teh temporary set back into the elective set

        num_net_req = len(network_req)
        num_net_elect = len(network_elect)
        tot_net = num_net_req + num_net_elect
        all_net_courses = network_req.union(network_elect) #UNION: createa new set with ALL network courses
